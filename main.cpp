...
void draw_bboxes(const cv::Mat& bgr, const std::vector<BoxInfo>& bboxes, object_rect effect_roi)
{
    ...
    cv::imwrite("image_test.jpg", image);
}


int image_demo(TinyDet &detector, const char* imagepath)
{
...
int webcam_demo(TinyDet& detector, int cam_id)
{
    cv::Mat image;
...
int video_demo(TinyDet& detector, const char* path)
{
    cv::Mat image;
...
int benchmark(TinyDet& detector)
{
    int loop_num = 100;
    int warm_up = 8;
...
int main(int argc, char** argv)
{
    ...
    TinyDet detector = TinyDet("../nanodet_m.param", "../nanodet_m.bin", true);
